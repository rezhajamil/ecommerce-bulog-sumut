                            ...
                            <div class="relative flex flex-col min-w-0 break-words border shadow lg:py4 rounded-2xl">
                                <div class="p-6 pb-0 mb-0">
                                    <div class="flex justify-between mb-2">
                                        ...
                                        <a href="{{ route('home') }}" class="text-sm underline text-secondary">Back to
                                            Home</a>
                                    </div>
                                </div>
                                <div class="flex-auto p-6">
                                    @if ($user)
                                        <form role="form" action="{{ route('user.forgot_password.store') }}"
                                            method="POST">
                                            @csrf
                                            <input type="hidden" name="user" value="{{ $user->id }}">
                                            <div class="mb-4">
                                                <p class="text-sm font-semibold text-primary">
                                                    {{ $user->security_question }}</p>
                                            </div>
                                            <div class="mb-4">
                                                <input type="text" placeholder="Jawaban Pertanyaan Keamanan"
                                            ...
                                            <div class="text-center">
                                                <button type="submit"
                                                    class="active:opacity-85 hover:shadow-xs tracking-tight-rem bg-150 bg-x-25 mb-0 mt-6 inline-block w-full cursor-pointer rounded-lg border-0 bg-primary px-16 py-3.5 text-center align-middle text-sm font-bold leading-normal text-white shadow-md transition-all ease-in hover:-translate-y-px">Submit</button>
                                            </div>
                                        </form>
                                    @else
                                        <form role="form" action="{{ route('user.forgot_password.create') }}"
                                            method="get">
                                            <div class="mb-4">
